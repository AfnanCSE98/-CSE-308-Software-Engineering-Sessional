...
public class mediator_org implements mediator{
  ...
  public void request(org ob , String service)
  {
    ...
    if(service.equalsIgnoreCase(ob.get_service())){
      System.out.println("You can't request for your own service");
    }
    else{
  ...
  public void serve(org ob){
    ...
    Pair p = new Pair();
    if(requests[idx].size()>0){
      p  = requests[idx].get(0);
  ...
  public int get_index(String service){
    int idx=0;
    for(int i=0;i<no_of_org;i++){
      if(service.equalsIgnoreCase(orgs.get(i).get_service())){
        idx = i;break;
      }
    }
    return idx;
